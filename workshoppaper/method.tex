...
X &\mapsto \sum_{d = 1}^D g(\sigma^d( X), c)
\end{align}
where $\sigma^d ( X)$ is the $d$-th singular value of $ X$ and
\begin{align}
...
\widehat {\mathcal S}_{GT}  &= \arg \min_{\mathcal S \in \binom{[P]}{d}} l_c ( X_{.\mathcal S})
\end{align}
...
We also claim that the conditions of the consequent of Proposition \ref{prop:orthonormal_basis} are satisfied by minimizers of the multitask basis pursuit objective applied to suitably normalized matrices in the special case where both such a submatrix exists and $|\mathcal S| = D$.
...
Since cannot in general ensure either that $|\widehat { \mathcal S|} = D$ or that a orthonormal submatrix $X_{.S}$ exists, we first use the convex problem to prune and then apply brute search upon the substantially reduced feature set.
