...
Our goal is, given a matrix $ X \in \mathbb R^{D \times P}$, select a subset $\mathcal S \subset [P]$ with $|\mathcal S| = D$ such that $X_{. \mathcal S}$ is as orthonormal as possible in a computationally efficient way.
...
\arg \min_{\mathcal S \subseteq [P] : |\mathcal S| = D} l ( X_{.\mathcal S})
\end{align}
