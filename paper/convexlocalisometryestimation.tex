...
A map $\phi$ between $D$ dimensional submanifolds with inherited Euclidean metric $\mathcal M \subseteq R^F$ and $\mathcal N  \subseteq R^E$ is an \textbf{isometry at a point} $p \in \mathcal M$ if
\begin{align}
u^T v = u^T {D \phi (p)}^T D \phi (p) \forall u,v \in \mathcal T_p \mathcal M
\end{align}
where $T_p \mathcal M$ is any basis for the tangent space of $\mathcal M$ at $p$ and $D \phi$ is the differential $[\frac{\partial}{\partial}]$ in those coordinates.
\end{definition}
